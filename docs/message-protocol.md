...
| `marketplace:assetState` | `{ assetId, installed, hasUpdate, autoUpdate, installedVersion?, availableVersion? }` | Drives the card: Install vs Update+Uninstall, and the auto-update checkbox |
...
| `marketplace:uninstall` | `{ assetId }` | Uninstall button (installed state) |
| `marketplace:setAutoUpdate` | `{ assetId, enabled }` | Per-asset auto-update checkbox |
...
| `webview:error` | `{ error }` | Global webview error relayed to the output channel |

## DTOs

```ts
interface CatalogAsset { id; type; name; version; description; tags?; source? }
interface AssetState   { installed; hasUpdate; autoUpdate; installedVersion?; availableVersion? }
