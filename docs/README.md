...
- **Assets are exported to native Copilot files.** When you "install" an asset, the
  extension writes **one flat file per asset** into `.github/` (`prompts/<id>.prompt.md`,
  `chatmodes/<id>.chatmode.md`, `instructions/<id>.instructions.md`) so **GitHub Copilot
  picks it up natively** — Agent Studio doesn't intercept Copilot, it _feeds_ it.
