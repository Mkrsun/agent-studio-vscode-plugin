...
3. You should see the assets from your configured content repo. **Install** one (the card
   shows **Install**, then **Update/Uninstall** + an **Auto-update** checkbox once
   installed) and check that a flat file appears, e.g.
   `.github/instructions/<id>.instructions.md`, in the dev window's workspace.
4. Open Copilot Chat and type `@agent-studio /discover` to exercise the chat participant.

> **See what's happening:** run **`Agent Studio: Show Logs`** to open the *Agent Studio*
> output channel — activation, marketplace fetches, and any webview errors are logged
> there. It's the first place to look when something doesn't appear.
