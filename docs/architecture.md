...
```
activate()
  1. loadDotEnv()            # .env → process.env (before anything reads overrides)
  2. dev bypass?             # only in Extension Dev Host + flag
  3. AuthService + sign-in   # always-on commands + placeholder view
  4. reconcile(state)        # toggles authenticated context key
  5. on authenticated →
       registerAuthenticatedSurface()
         enforceLatestVersion()      # self-update (≤1/day)
         MarketplaceService.init()   # parallel registry fetches
         AssetLoader.loadAll()       # + auto-update repo-scoped assets
         InspectorProvider           # sidebar tree
...
MarketplaceService  ──refs──▶  AssetLoader  ──parsed assets──▶  in-memory catalog
                ...
                ScopeService (session | repo | disabled) + installed version
                                   │  repo-scoped only
                                   ▼
                         CopilotExporter ──writes──▶ .github/{prompts,chatmodes,instructions,copilot-instructions.md}
                                   │
                                   ▼
