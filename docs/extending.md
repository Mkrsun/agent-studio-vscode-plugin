...
All export rendering is in `src/services/copilotExporter.ts`: the flat per-asset paths
(`getAssetExportPath`), the per-type builders, the `source:` frontmatter, and the
`_reconcile`/`_sweep` cleanup all live there. Changing this affects how Copilot consumes
assets — update [SDD §4.4](./SDD.md#44-asset-pipeline-servicesassetloaderts-scopeservicets-copilotexporterts)
