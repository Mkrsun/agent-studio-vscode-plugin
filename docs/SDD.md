...
### 4.1 Activation & lifecycle (`extension.ts`, `auth/authGate.ts`)

`activate()` runs the **boot sequence**:

1. `loadDotEnv(context)` — merge `.env` (workspace folders, then extension dir) into
   `process.env`. Real env always wins. **Must run before any service reads an override.**
2. Dev bypass: in Extension Development Host + `auth.bypassForDev`, skip auth and register
   the full surface immediately (never effective in a packaged `.vsix`).
3. Create `AuthService`; register the always-on commands (`signIn`, `signOut`,
   `showAuthStatus`) and the sign-in placeholder view.
4. `reconcile(state)` toggles the `agentStudio.authenticated` context key and swaps the
   sign-in view for the **authenticated surface** when a session appears.
5. `registerAuthenticatedSurface()` wires every service, the inspector, the chat
   participant, all commands, and kicks off self-update + asset auto-update.
...
- **`ScopeService`** assigns each asset a scope:
  ...
  It also records the **installed version** per asset (for update detection).
- **`CopilotExporter`** renders repo-scoped assets into native Copilot files:

  | Asset type | Exported file | Copilot feature |
  |------------|---------------|-----------------|
  | skill | `.github/prompts/as-skill-<id>.prompt.md` | reusable prompt |
  | agent | `.github/chatmodes/as-agent-<id>.chatmode.md` | custom chat mode |
  | instruction | `.github/instructions/as-inst-<id>.instructions.md` | glob-injected |
  | workflow | `.github/prompts/as-workflow-<id>.prompt.md` | guided prompt |
  | hook (global) | aggregated into `.github/copilot-instructions.md` | repo-wide context |

- **`assetAutoUpdate.autoUpdateAssets()`** re-exports repo-scoped assets when the registry
  version is newer than the recorded installed version (on load and on catalog refresh).
...
categories → asset nodes, plus Plugins and MCP sections. Node icons reflect scope/status.
Context-menu commands (`enable`, `disable`, `preview`, `inject`) operate through
`ScopeService` + `CopilotExporter` and refresh the tree via its change event.
...
- **Configurability.** See [Configuration](./configuration.md). Everything routable is
  `env → setting → default`.
- **Observability.** User-facing toasts for outcomes; the analytics layer is the
  longitudinal signal.
...
- **`installer.ts` (`AssetInstaller`)** overlaps with `CopilotExporter`; constructed in
  `authGate` but unused there — candidate for consolidation.
- **`marketplace:preview`** is a stub on the webview path (the inspector has a working
  preview command).
